...
export function ProjectCard({ project }: { project: Project }) {
    ...
    return (
        <div className="relative h-full border border-zinc-800 rounded-xl ">
            <MagicCard
                blur={0.5}
                borderWidth={1}
                spread={180}
            ...
            />
            <Link
                className=" flex gap-2 text-base  tracking-tight "
                scroll={false}
                href={`/projects/${slug}`}
            >
                <article className=" group/image flex flex-col w-full p-4  space-y-2">
